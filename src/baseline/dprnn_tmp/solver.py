...
class Solver(object):
    ...
    def _run_one_epoch(self, data_loader, validation=False):
        total_loss = 0
        for i, (a_mix, a_tgt) in enumerate(data_loader):
            a_mix = a_mix.cuda().squeeze().float()
            a_tgt_ref = a_tgt.cuda().squeeze().float()

            estimate_source = self.model(a_mix)
            max_snr = cal_si_snr_with_pit(a_tgt_ref, estimate_source)
