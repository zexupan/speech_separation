...
class Dual_RNN_Block(nn.Module):
    ...
    def __init__(self, out_channels,
        ...
        self.intra_att = multihead_ca(d_model= out_channels, nhead = 4, att_dropout=dropout)
        self.inter_att = multihead_ca(d_model= out_channels, nhead = 4, att_dropout=dropout)
        self.intra_norm_att = nn.LayerNorm(out_channels)
        self.inter_norm_att = nn.LayerNorm(out_channels)
    ...
    def forward(self, x):
        ...
        # att
        intra_att = self.intra_att(intra_rnn)
        intra_rnn = intra_rnn + intra_att
        intra_rnn = self.intra_norm_att(intra_rnn)
        ...
        #att
        inter_att = self.inter_att(inter_rnn)
        inter_rnn = inter_rnn + inter_att
        inter_rnn = self.inter_norm_att(inter_rnn)
